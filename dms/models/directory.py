...
class DmsDirectory(models.Model):
    ...
    def action_open_share_wizard(self):
        ...
        return {
            ...
            'res_model': 'dms.directory.share.wizard',
            'view_mode': 'form',
            'views': [(self.env.ref('dms.view_dms_directory_share_wizard_form').id, 'form')],
            'target': 'new',
    ...
    def create(self, vals_list):
        admin_group = self.env.ref("dms.group_dms_manager")  # Adjust if your admin group XML ID is different
        current_user = self.env.user

        for vals in vals_list:
            # --- Check if trying to create a root directory ---
            if not vals.get("parent_id"):
                # Only Admin group can create root directories
                if admin_group.id not in current_user.groups_id.ids:
                    raise UserError(_("Only Admin users can create root directories."))

            # If creating a subdirectory, inherit storage from parent
            else:
    ...
    def write(self, vals):
        # Restrict changing storage/parent for consistency
        if any(k in vals.keys() for k in ["storage_id", "parent_id"]):
            for item in self:
                ...
                old_storage_id = (item.storage_id or item.root_directory_id.storage_id).id
                if new_parent_id:
        ...
        # Groups part: propagate changes to children
...
class DmsPublicController(http.Controller):

    @http.route('/dms/public/directory/<int:directory_id>', type='http', auth='public', website=True)
    def public_directory(self, directory_id, access_token=None, **kwargs):
        directory = request.env['dms.directory'].sudo().search([
            ('id', '=', directory_id),
        ...
        # Fetch files in the directory to show
        files = request.env['dms.file'].sudo().search([('directory_id', '=', directory.id)])

        # If POST and file upload present, handle upload
        if request.httprequest.method == 'POST':
            uploaded_file = request.httprequest.files.get('file')
            if uploaded_file:
                # create file record inside the directory
                request.env['dms.file'].sudo().create({
                    'name': uploaded_file.filename,
                    'directory_id': directory.id,
                    'datas': uploaded_file.read().encode('base64'),  # You may need to adapt encoding
                    'datas_fname': uploaded_file.filename,
                    'mimetype': uploaded_file.content_type,
                })
                return request.redirect(request.httprequest.url)

        # Render a simple template to show files and upload form
        return request.render('dms.public_directory_template', {
            'directory': directory,
            'files': files,
            'access_token': access_token,
        })