...
class CustomerPortal(CustomerPortal):
    ...
    def _dms_check_access(self, model, res_id, access_token=None):
        ...
        if access_token:
            ...
            if model == "dms.directory":
                ...
                current = item
                while current:
                    if current.access_token and access_token == current.access_token:
                        return item
                    current = current.parent_id
                return False

            elif model == "dms.file":
                # Check the file's directory and its parent directories
                current = item.directory_id
                while current:
                    if current.access_token and access_token == current.access_token:
                        return item
                    current = current.parent_id
            ...
            # No token: check user permissions
            ...
            if model == "dms.directory":
                ...
                if not item.directory_id.permission_read:
                    return False
            else:
                return False
    ...
    @http.route(['/my/dms/directory/<int:dms_directory_id>/share'], type='http', auth='user', methods=['POST'], website=True)
    def share_dms_directory(self, dms_directory_id, email=None, **kwargs):
        # Validate email input
        if not email:
            return request.redirect(f"/my/dms/directory/{dms_directory_id}?error=missing_email")

        # Generate or get access token for the directory
        directory = request.env['dms.directory'].sudo().browse(dms_directory_id)
        if not directory.exists():
            return request.redirect('/my/dms')

        # Generate or fetch a unique access token for this share (you should implement token generation)
        if not directory.access_token:
            directory.access_token = directory._generate_access_token()  # You need to implement _generate_access_token

        access_token = directory.access_token

        # Prepare the share URL
        share_url = f"{request.httprequest.host_url}my/dms/directory/{dms_directory_id}?access_token={access_token}"

        # Compose email body
        email_body = f"""
        <p>Hello,</p>
        <p>You have been invited to access the folder: <strong>{directory.name}</strong>.</p>
        <p>Click the link below to view the folder:</p>
        ...
        # Send email
        mail_values = {
            'subject': f'Invitation to access folder {directory.name}',
            'body_html': email_body,
            'email_to': email,
        }
        mail_mail = request.env['mail.mail'].sudo().create(mail_values)
        mail_mail.send()

        # Redirect back to the directory page with success message
        return request.redirect(f"/my/dms/directory/{dms_directory_id}?success=share_sent")

    @http.route('/my/dms/directory/upload', type='http', auth='public', methods=['POST'], csrf=False, website=True)
    def portal_dms_directory_upload(self, directory_id=None, access_token=None, **post):
        """Upload files into directory, if access token is valid (on directory or parent)."""
        directory = request.env['dms.directory'].sudo().browse(int(directory_id or 0))
        if not directory.exists():
            return request.redirect('/my/dms')
        ...
        if request.env.user._is_public():
            ...
            while current:
                ...
                current = current.parent_id
            if not valid:
                return request.redirect(f'/my/dms/directory/{directory_id}?error=access_denied')

        uploaded_files = request.httprequest.files.getlist('files')
        if not uploaded_files:
            return request.redirect(f'/my/dms/directory/{directory_id}?access_token={access_token}&error=no_files')
        ...
        for f in uploaded_files:
            ...
            _logger.info(f"Uploading file '{name}' into directory {directory_id} with token {access_token}")
        ...
        return request.redirect(f'/my/dms/directory/{directory_id}?access_token={access_token}&success=upload_done')